...
*/
#ifndef MXNET_OPERATOR_BINARYCONVOLUTIONPLUS_INL_H_
#define MXNET_OPERATOR_BINARYCONVOLUTIONPLUS_INL_H_
...
namespace conv {
enum BinaryConvolutionOpInputs {kData, kWeight, kBias};
enum BinaryConvolutionOpOutputs {kOut};
enum BinaryConvolutionOpResource {kTempSpace};
enum BinaryConvolutionOpCudnnTune {kOff, kLimited, kFastest};
}

struct BinaryConvolutionPlusParam : public dmlc::Parameter<BinaryConvolutionPlusParam> {
    TShape kernel;
    TShape stride;
    TShape dilate;
    TShape pad;
    ...
    DMLC_DECLARE_PARAMETER(BinaryConvolutionPlusParam) {
...
class BinaryConvolutionPlusOp : public Operator {
public:
    explicit BinaryConvolutionPlusOp(BinaryConvolutionPlusParam p) {
        this->param_ = p;
        ...
        for (index_t i = 0; i < nbatch; i += nstep_) {
            ...
            for (uint32_t gid = 0; gid < param_.num_group; ++gid) {
                mshadow::Tensor<xpu, 2, DType> tmpc = temp_col.Slice(gstride * gid,
                                                                     gstride * (gid + 1));
                mshadow::Tensor<xpu, 2, DType> tmp_wmat = wmat[gid];
                mshadow::Tensor<xpu, 2, DType> tmp_dst = temp_dst[gid];
                BinaryConvolutionPlusForward(wmat[gid], tmpc, temp_dst[gid]);
                //BinaryConvolutionForward(tmp_wmat, tmpc, tmp_dst);
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        for (index_t i = 0; i < nbatch; i += nstep_) {
            ...
            for (uint32_t gid = 0; gid < param_.num_group; ++gid) {
                ...
                BinaryConvolutionPlusApproximate(tmpc);
                if (i == 0) {
            ...
            for (uint32_t gid = 0; gid < param_.num_group; ++gid) {
                Tensor<xpu, 2, DType> tmpc = temp_col.Slice(gstride * gid, gstride * (gid + 1));
                BinaryConvolutionPlusBackward(wmat[gid], temp_dst[gid], tmpc);
...
private:
    ...
    BinaryConvolutionPlusParam param_;
...
Operator* CreateOp(BinaryConvolutionPlusParam param, int dtype,
                   std::vector<TShape> *in_shape,
                   std::vector<TShape> *out_shape,
                   Context ctx);

#if DMLC_USE_CXX11
class BinaryConvolutionPlusProp : public OperatorProperty {
 public:
  ...
  OperatorProperty* Copy() const override {
    auto ptr = new BinaryConvolutionPlusProp();
    ptr->param_ = param_;
    return ptr;
  }

  std::string TypeString() const override {
    return "BinaryConvolutionPlus";
  }
