...
*/
#ifndef MXNET_OPERATOR_BINARYFULLYCONNECTEDPLUS_H_
#define MXNET_OPERATOR_BINARYFULLYCONNECTEDPLUS_H_
...
namespace fullc {
enum BinaryFullyConnectedOpInputs {kData, kWeight, kBias};
enum BinaryFullyConnectedOpOutputs {kOut};
}  // fullc

struct BinaryFullyConnectedPlusParam : public dmlc::Parameter<BinaryFullyConnectedPlusParam> {
    int num_hidden;
    bool no_bias;
    DMLC_DECLARE_PARAMETER(BinaryFullyConnectedPlusParam) {
...
class BinaryFullyConnectedPlusOp : public Operator {
public:
    explicit BinaryFullyConnectedPlusOp(BinaryFullyConnectedPlusParam p) {
        this->param_ = p;
    }
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        //printf("binary-fullyconnectedPlus start!\n");
        BinaryFullyConnectedPlusForward(data, wmat, out);
        if (!param_.no_bias) {
            Tensor<xpu, 1, DType> bias = in_data[fullc::kBias].get<xpu, 1, DType>(s);
            out += repmat(bias, data.size(0));
        }
        //printf("binary-fullyconnectedPlus end!\n");
        /*
        Tensor<xpu, 2, DType> wbmat = binary(wmat);

        out = dot(data, wbmat.T());
        FreeSpace(&wbmat); // for that this binary is a copy version
        */
    }
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        Tensor<xpu, 2, DType> gdata = in_grad[fullc::kData].get_with_shape<xpu, 2, DType>(
                Shape2(ishape[0], ishape.ProdShape(1, ishape.ndim())), s);
        //printf("nFC back1\n");
        BinaryFullyConnectedPlusBackward(data, grad, wmat, gwmat, gdata);
        //printf("nFC back2\n");
        /*
        Tensor<xpu, 2, DType> wbmat = binary(wmat, true);
        Assign(gdata, req[fullc::kData], dot(grad, wbmat));
        FreeSpace(&wbmat);
         */
    }

private:
    BinaryFullyConnectedPlusParam param_;
};  // class FullyConnectedOp

// Decalre Factory function, used for dispatch specialization
template<typename xpu>
Operator* CreateOp(BinaryFullyConnectedPlusParam param, int dtype);

#if DMLC_USE_CXX11
class BinaryFullyConnectedPlusProp : public OperatorProperty {
 public:
  ...
  OperatorProperty* Copy() const override {
    BinaryFullyConnectedPlusProp* fc_sym = new BinaryFullyConnectedPlusProp();
    fc_sym->param_ = this->param_;
    return fc_sym;
  }

  std::string TypeString() const override {
    return "BinaryFullyConnectedPlus";
  }
