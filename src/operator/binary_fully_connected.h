...
struct BinaryFullyConnectedParam : public dmlc::Parameter<BinaryFullyConnectedParam> {
    int num_hidden;
    bool no_bias;
    DMLC_DECLARE_PARAMETER(BinaryFullyConnectedParam) {
...
class BinaryFullyConnectedOp : public Operator {
public:
    explicit BinaryFullyConnectedOp(BinaryFullyConnectedParam p) {
        this->param_ = p;
    }
    ...
    virtual void Forward(const OpContext &ctx,
        ...
        BinaryFullyConnectedForward(data, wmat, out);
        if (!param_.no_bias) {
    ...
    virtual void Backward(const OpContext &ctx,
        ...
        Tensor<xpu, 2, DType> gdata = in_grad[fullc::kData].get_with_shape<xpu, 2, DType>(
                Shape2(ishape[0], ishape.ProdShape(1, ishape.ndim())), s);
        //printf("nFC back1\n");
        BinaryFullyConnectedBackward(data, grad, wmat, gwmat, gdata);
        //printf("nFC back2\n");
        /*
...
private:
    BinaryFullyConnectedParam param_;
};  // class FullyConnectedOp

// Decalre Factory function, used for dispatch specialization
template<typename xpu>
Operator* CreateOp(BinaryFullyConnectedParam param, int dtype);

#if DMLC_USE_CXX11
class BinaryFullyConnectedProp : public OperatorProperty {
 public:
  ...
  OperatorProperty* Copy() const override {
    BinaryFullyConnectedProp* fc_sym = new BinaryFullyConnectedProp();
    fc_sym->param_ = this->param_;
    return fc_sym;
  }

  std::string TypeString() const override {
    return "BinaryFullyConnected";
  }
